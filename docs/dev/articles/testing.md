...
[`bio/seq`](https://klmr.me/box/dev/articles/bio/seq.r) module from the
*[Getting started](https://klmr.me/box/dev/articles/box.md)* vignette.
To enable unit testing, this module contains the following code at the
very end:
...
“Jobs” › “Start Local Job…” [¹](#fn1). This works because
[`box::name`](https://klmr.me/box/dev/reference/name.md) returns the
name of the module from which this function is called. But if the
function is invoked from code that wasn’t loaded via
[`box::use`](https://klmr.me/box/dev/reference/use.md) (as is the case
...
- `__tests__/`
  - [`__init__.r`](https://klmr.me/box/dev/articles/bio/__tests__/__init__.r)
  - [`helper-module.r`](https://klmr.me/box/dev/articles/bio/__tests__/helper-module.r)
...
[`box::file`](https://klmr.me/box/dev/reference/file.md) inside the
module’s `.on_load` hook — remember that only declarations should be at
file level inside a module! All code execution should happen inside
functions.
...
[`box::use`](https://klmr.me/box/dev/reference/use.md) doesn’t reload
...
internal module implementation.[²](#fn2)
...
2.  Get access to the private module namespace for testing. When loading
    a module with
    [`box::use`](https://klmr.me/box/dev/reference/use.md), the module
    object has an attribute `namespace` that holds the private module
    namespace. It generally shouldn’t be accessed by client code, but
    access by the testing code for a module is entirely legitimate:
...
2.  Depending on how it’s invoked; but we shouldn’t make assumptions. In
    particular, when invoked as a job in RStudio, the test module is
