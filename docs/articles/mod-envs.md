...
loaded modules (as well as packages loaded via
[`box::use`](https://klmr.me/box/reference/use.md)) correspond to a mesh
of several interconnected environments.
...
Suraj Gupta, and
[*Environments*](http://adv-r.had.co.nz/Environments.md) in *Advanced R*
by Hadley Wickham. The following assumes that the reader has a more than
passing familiarity with these concepts.
...
functionality. For packages that are loaded by
[`box::use`](https://klmr.me/box/reference/use.md), the rules are the
same as for modules.
...
attachment declarations in
[`box::use`](https://klmr.me/box/reference/use.md) expressions.
...
module. If users create a module alias in their
[`box::use`](https://klmr.me/box/reference/use.md) call, the alias will
be a reference to this environment.
...
[`box::use`](https://klmr.me/box/reference/use.md) with a non-empty
attach list in `.GlobalEnv` is therefore similar to calling `library`.
In particular, inside `.GlobalEnv` the call `box::use(pkg[...])` in
pretty much equivalent to calling
[`library(pkg)`](https://rdrr.io/r/base/library.html), and is
discouraged for the same reason that
[`library(pkg)`](https://rdrr.io/r/base/library.html) is discouraged.
