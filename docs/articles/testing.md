...
[`bio/seq`](https://klmr.me/box/articles/bio/seq.r) module from the
*[Getting started](https://klmr.me/box/articles/box.md)* vignette. To
enable unit testing, this module contains the following code at the very
end:
...
“Jobs” › “Start Local Job…” [^1]. This works because
[`box::name`](https://klmr.me/box/reference/name.md) returns the name of
the module from which this function is called. But if the function is
invoked from code that wasn’t loaded via
[`box::use`](https://klmr.me/box/reference/use.md) (as is the case
here), its value is `NULL`. In other words, `is.null(box::name())` is a
way of testing whether the code currently being run is loaded as a
...
- `__tests__/`
  - [`__init__.r`](https://klmr.me/box/articles/bio/__tests__/__init__.r)
  - [`helper-module.r`](https://klmr.me/box/articles/bio/__tests__/helper-module.r)
...
[`box::file`](https://klmr.me/box/reference/file.md) inside the module’s
`.on_load` hook — remember that only declarations should be at file
level inside a module! All code execution should happen inside
functions.
...
[`box::use`](https://klmr.me/box/reference/use.md) doesn’t reload code
that has already been loaded previously; instead, it uses an already
loaded, cached version. This means that running the tests via the
“Source” button risks running an outdated version of the tests, or the
module, or both, after modifying their code.
...
2.  Get access to the private module namespace for testing. When loading
    a module with [`box::use`](https://klmr.me/box/reference/use.md),
    the module object has an attribute `namespace` that holds the
    private module namespace. It generally shouldn’t be accessed by
    client code, but access by the testing code for a module is entirely
    legitimate:
