...
is_module_loaded = function (mod) {
    pod:::is_mod_loaded(attr(mod, 'info'))
}
...
test_that('import works in global namespace', {
    in_globalenv({
        pod::use(mod/a)
        expect_true(pod:::is_mod_loaded(attr(a, 'info')))
        expect_true('double' %in% ls(a))
    })
})
...
test_that('modules don’t need exports', {
    ...
    expect_error(pod::use(mod/no_exports), NA)
    expect_error(capture.output(pod::use(mod/no_names)), NA)
...
test_that('global scope is not leaking into modules', {
    in_globalenv({
        x = 1L
        expect_error(pod::use(mod/issue151), 'object .* not found')
    })
})
