...
xyz::set_script_path(file.path(tmpdir, 'c'))
```

## Current status

The ‘xyz’ package doesn’t have a built-in foreign function interface yet but it
is possible to integrate compiled code via R’s `SHLIB` mechanism for building
shared objects. In particular, this also works with packages such as [Rcpp][].
...
2. Load the compiled shared object when a module is loaded.
...
`__install__`. This convention makes it clear that this is a “special” module,
...
```{r file='c/__install__.r'}
```

In principle, only the first line in this file should need to be changed for
other C projects.

With this in place, we can invoke the compilation by loading the `c/__install__`
submodule:

```{r}
xyz::use(c/`__install__`)
```

The result of the compilation will be a single file, `hello.so` (on Unix and
macOS) or `hello.dll` (on Windows) which represents a *shared object file*, and
which we can load and use inside R.

## Loading compiled code

Compiled code from a shared object file is loaded in R using the `dyn.load`
command. This will happen inside the module that uses and/or exposes the
compiled code.

Since the name of the shared object file is platform dependent, we need a helper
function that gives us this name:
...
Now our module can load the compiled code:
