...
mod::set_script_path(file.path(tmpdir, 'c'))
```

## Current status

Modules don’t have a built-in foreign function interface yet but it is possible
to integrate compiled code via R’s `SHLIB` mechanism for building shared
objects. In particular, this also works with packages such as [Rcpp][].
