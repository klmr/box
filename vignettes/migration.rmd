...
Unlike with `modules::import()`, `box::use()` allows (and encourages) multiple
...
‘box’ loads documentation lazily only when it is requested via `box::help()`
(see below).
...
Instead, ‘box’ introduces [module event hooks][hooks], in particular
...
The ‘modules’ function `module_file` is now called [`box::file()`][box::file].
...
`@export` directives that decorate `box::use()` declarations apply to *all*
