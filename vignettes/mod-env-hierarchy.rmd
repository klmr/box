...
> not required knowledge for users of the {mod} package and module authors.
...
packages loaded via `mod::use`) correspond to a mesh of several interconnected
environments.
...
packages that are loaded by `mod::use`, the rules are the same as for modules.
...
code that loaded the package. By contrast, {mod} needs to distinguish the set of
...
`mod::use` with a non-empty attach list in `.GlobalEnv` is therefore similar to
calling `library`. In particular, inside `.GlobalEnv` the call
`mod::use(pkg[...])` in pretty much equivalent to calling `library(pkg)`, and is
