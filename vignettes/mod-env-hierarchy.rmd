...
> not required knowledge for users of the ‘xyz’ package and module authors.
...
packages loaded via `xyz::use`) correspond to a mesh of several interconnected
environments.
...
packages that are loaded by `xyz::use`, the rules are the same as for modules.
...
create a module alias in their `xyz::use` call, the alias will be a reference to
this environment.
...
code that loaded the package. By contrast, ‘xyz’ needs to distinguish the set of
...
`xyz::use` with a non-empty attach list in `.GlobalEnv` is therefore similar to
calling `library`. In particular, inside `.GlobalEnv` the call
`xyz::use(pkg[...])` in pretty much equivalent to calling `library(pkg)`, and is
