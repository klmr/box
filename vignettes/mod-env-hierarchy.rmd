...
> not required knowledge for users of the ‘pod’ package and module authors.
...
packages loaded via `pod::use`) correspond to a mesh of several interconnected
environments.
...
packages that are loaded by `pod::use`, the rules are the same as for modules.
...
The loaded modules can be represented by the following schematic, with each pod
...
create a module alias in their `pod::use` call, the alias will be a reference to
this environment.
...
code that loaded the package. By contrast, ‘pod’ needs to distinguish the set of
...
`pod::use` with a non-empty attach list in `.GlobalEnv` is therefore similar to
calling `library`. In particular, inside `.GlobalEnv` the call
`pod::use(pkg[...])` in pretty much equivalent to calling `library(pkg)`, and is
