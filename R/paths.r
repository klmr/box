...
#' code can be located and will be found by \pkg{pod}.
#'
...
#' \code{getOption('pod.path')} specifies the search path as a character vector.
#' Users can override its value by separately setting the environment variable
#' \env{R_POD_PATH} to one or more paths, separated by the platform’s path
#' separator (\dQuote{:} on UNIX-like systems, \dQuote{;} on Windows).
#' @keywords internal
#' @name paths
mod_search_path = function (caller) {
    env_value = strsplit(Sys.getenv('R_POD_PATH'), .Platform$path.sep)[[1L]]
    c(env_value %||% getOption('pod.path'), calling_mod_path(caller))
}

#' \code{calling_mod_path} determines the path of the module code that is
#' currently calling into the \pkg{pod} package.
#'
#' @param caller the environment from which \code{pod::use} was invoked.
#' @return \code{calling_mod_path} the path of the source module that is calling
#' \code{pod::use}, or the script’s path if the calling code is not a module.
#' @rdname paths
