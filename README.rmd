...
pak::pkg_install('klmr/xyz@main')
```

## 🥜 Usage in a nutshell

‘xyz’ provides a better way of organising R code in a modular fashion. It allows
loading just the parts of packages that you need, and giving imported names any
alias you want. And it simplifies *writing modular code* by treating files and
folders of R code as independent modules, without requiring the user to wrap
reusable code into packages.

‘xyz’ replaces the base R `library` function with `xyz::use`. `xyz::use` is more
...
Instead, we can also instruct `xyz::use` to not attach any names when loading a
...
The following `xyz::use` declaration illustrates all these different cases:
...
Furthermore, unlike with `library`, the effects of `xyz::use` are restricted to
...
Unlike other packages, ‘xyz’ itself is never loaded via `library`. Instead, its
functionality is always used explicitly via `xyz::use`.
...
‘xyz’ promotes a philosophy that is in some way the opposite of what’s common in
...
best practices dictate that a given scope should contain as few names as
possible, and to make them as explicit as possible.
...
symbols`, which correspond to `xyz::use(pkg)` and `xyz::use(pkg[a, few,
symbols])`, respectively.

‘xyz’ also makes it drastically easier to *write* reusable code: instead of
needing to create a package, each R code file *is already a module* which can be
imported using `xyz::use`. Modules can also be nested inside directories, such
that self-contained projects can be easily split into separate or interdependent
submodules.

For more information refer to the [Basic usage][] vignette.

[pep8]: https://www.python.org/dev/peps/pep-0008/#id23
[Basic usage]: articles/basic-usage.html
